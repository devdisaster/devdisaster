...
1. Build the R9 controlled Stripe gateway routes: `POST/GET /demo/stripe/v1/payment_intents[...]` forwarding to real Stripe test mode and shaping per `activeContractVersion`, with `Stripe-Version` response header and `cachedResponse` fallback; `GET /demo/stripe/docs` rendering the matching contract; `vendor.resetV1` / `vendor.upgradeV2` mutations.
   **Verified 30 Aug 2026:** complete on the personal `famous-bass-25` development deployment. Real Stripe test-mode POST and GET calls produced the versioned V1 and V2 contracts; the deterministic docs mirror flipped from the old `charges` reference to the faithful `latest_charge` removal changelog from the same integration flag. The persisted fallback was sanitized, reshaped for the active version, marked in the response, and emitted one operational event during a controlled connectivity failure. Version mutations were idempotent and emitted only one event per actual change. `npm run lint`, `npm run build`, and `npx convex dev --once` passed; verification finished on `2022-08-01`, and incident/session counts remained unchanged with no monitor configured.
